...
import math
import sys
from config import bazne_cijene_mrs
...
class MRS_Parser:
    ...
    def parse_single_qos(self, path_to_folder, user, sheet_name):  
        ...
        # ram usage metrics
        ram_capacity =  pd.Series([float(ginfo['Available RAM']) for i in range(15)])
        metrics_avg['Used storage(image size, GB)'] = pd.Series([float(ginfo['HDD']) for i in range(15)])
        metrics_avg['Min vCPU'] = pd.Series([float(ginfo['Vcpus']) for i in range(15)])
        metrics_avg['Min RAM'] = pd.Series([float(float(ginfo['RAM'].strip())/1024.0) for i in range(15)])
        metrics_avg['Users'] = pd.Series([user for i in range(15)])

        # memory data
        data = pd.read_csv(path_to_folder + 'memory_unused' + self.extension, sep=' ', header=None, decimal=",", error_bad_lines=False)

        step = math.ceil(data[2].size/15)
        start = 0
        end = step
        ...
        for filename in os.listdir(path_to_folder):
            ...
            if root.startswith('jMeter') and not root.endswith('transaction') and ext == '.csv':
                ...
                transactionController = jMeter_data[jMeter_data['label'] == 'Transaction Controller']
                end_to_end = transactionController['elapsed']
                
                slo_length = end_to_end.shape[0]
                step = math.ceil(slo_length/15)
                start = 0
                end = step
                data_range = pd.Series()
                slo_violations = []
    ...
    def parse_single_cost(self, path_to_folder, user, sheet_name):
        ...
        for filename in os.listdir(path_to_folder):
            ...
            if root.startswith('jMeter') and not root.endswith('transaction') and ext == '.csv':
                ...
                # Http Request1
                httpReq1 = jMeter_data[jMeter_data['label'] == 'HTTP Request1']
                # Http Request2
                httpReq2 = jMeter_data[jMeter_data['label'] == 'HTTP Request2']
                # Http Request3
                httpReq3 = jMeter_data[jMeter_data['label'] == 'HTTP Request3']
                # Http Transaction Controller
                transactionController = jMeter_data[jMeter_data['label'] == 'Transaction Controller']
                
                metrics_avg['Request 1'] = httpReq1['elapsed'].mean()
                metrics_avg['Request 2'] = httpReq2['elapsed'].mean()
                metrics_avg['Request 3'] = httpReq3['elapsed'].mean()
                metrics_avg['End-to-end'] = transactionController['elapsed'].mean()
                #slo_viol
                violated = sum(i > 5000 for i in transactionController['elapsed'])
                transaction_len = len(transactionController['elapsed'])
    ...
    def calculate_price_qos(self, cost_info, input_sheet_name):
        ...
        for provider in providers:
            ...
            for index in range(15):
                return_cost[provider] = [0]*15

        #init provider cost
        provider_cost = {}
        for provider in providers:
            provider_cost[provider] = [0]*15
    ...
    def parse(self): 
        data = {}
        for db_instance in sorted(os.listdir(self.root_folder)):
            for mrs_instance in sorted(os.listdir(self.root_folder+"/"+db_instance)):
                tmp = []
                sheet_name = self.sheet_names[db_instance.split("_")[0] + "_" + "".join(mrs_instance.split("_")[1:])]

                for mrs_instance_users in sorted(os.listdir(self.root_folder+"/"+db_instance+"/"+mrs_instance)):
                    #print(mrs_instance_users)
                    path_to_folder = self.root_folder+"/"+db_instance+"/"+mrs_instance+"/"+mrs_instance_users+"/"
